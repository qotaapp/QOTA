-- ============================================================
-- QOTA — LE FEED (HOME) INCLUT MAINTENANT FIGURES PUBLIQUES ET
-- LES SECTIONS "CHAÎNES ET PROGRAMMES" / "VENTE EN LIGNE" / "AUTRES"
-- ============================================================
-- get_feed() excluait volontairement les Figures Publiques
-- ("hors périmètre" à l'époque, §35) et n'a jamais inclus les
-- Admin Listings (§022/§033). Ce n'est plus le cas : comme les
-- Services et les User Items, un contenu approuvé de ces types doit
-- apparaître dans le Feed. Reprend EXACTEMENT la version actuelle de
-- la fonction (028_feed_order_by_date.sql — views_count, owner_id,
-- owner_avatar_url, tri chronologique) : seule la clause
-- `where e.kind in (...)` change.
-- ============================================================
...
as $$
    ...
    base as (
        ...
        where e.status = 'active'
          and e.kind in ('service', 'user_item', 'public_figure', 'admin_listing')
    ),
    scored as (
        select
            b.*,
