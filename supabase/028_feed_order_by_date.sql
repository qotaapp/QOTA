-- Home doit afficher la publication la plus récente en premier —
-- remplace le tri algorithmique (score) par un tri chronologique
-- strict. Le calcul de `score` reste renvoyé (au cas où il serait
-- réutilisé ailleurs) mais n'est plus utilisé pour l'ordre final.
...
as $$
    ...
    base as (
        ...
        where e.status = 'active'
          and e.kind in ('service', 'user_item')
    ),
    scored as (
        select
            b.*,
    ...
    -- Tri chronologique strict : la publication la plus récente en
    -- premier (remplace l'ancien "order by score desc, created_at desc").
