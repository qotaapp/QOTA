-- ============================================================
-- QOTA — CORRECTIF FEED : inclure Chaînes/Programmes, Vente en
-- ligne, Autres (§026) dans la Home, en plus de service/user_item.
-- Une seule ligne change vs 012_feed_algorithm.sql : le filtre kind.
-- ============================================================
...
returns table (
    ...
    city_name_fr text,
    zone_name_fr text,
    owner_id uuid, -- rempli uniquement pour les User Items (§23)
    owner_name text,
    owner_avatar_url text, -- rempli uniquement pour les User Items (§23)
...
as $$
    ...
    base as (
        ...
        where e.status = 'active'
          -- Figures publiques toujours exclues (§35, hors périmètre) ;
          -- Chaînes/programmes, Vente en ligne, Autres (§026) ajoutées.
          and e.kind in ('service', 'user_item', 'chain_program', 'online_sale', 'other')
    ),
    scored as (
        select
            b.*,
