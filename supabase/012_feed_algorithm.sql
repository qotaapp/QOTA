-- ============================================================
-- QOTA — ALGORITHME DU FEED (§9-10)
-- "Le Feed ne doit PAS être uniquement chronologique. Le classement
--  doit prendre en compte : proximité géographique, fraîcheur,
--  nombre d'évaluations, engagement, pertinence, éventuellement
--  promotion payante. Le système doit permettre l'évolution future
--  de l'algorithme."
--
-- Conçu comme une fonction unique et isolée (get_feed) : les poids
-- sont des constantes modifiables sans toucher au reste du schéma,
-- et une V2 pourra la remplacer sans casser le contrat d'interface
-- (mêmes colonnes en sortie).
-- ============================================================

-- Promotion payante (§10) — activable depuis un futur Dashboard.
...
as $$
    ...
    base as (
        select
            ...
            e.is_promoted, e.promoted_until,
