...
-- contenu final (pas juste un "type"). Réexécutable sans erreur
-- même si une exécution précédente a partiellement réussi.
-- ============================================================
...
drop policy if exists "Public read active bon_plans" on bon_plans;
create policy "Public read active bon_plans" on bon_plans
    for select using (active = true or is_super_admin());

drop policy if exists "Admin manage bon_plans" on bon_plans;
create policy "Admin manage bon_plans" on bon_plans
    for insert with check (is_super_admin());

drop policy if exists "Admin update bon_plans" on bon_plans;
create policy "Admin update bon_plans" on bon_plans
    for update using (is_super_admin());

drop policy if exists "Admin delete bon_plans" on bon_plans;
